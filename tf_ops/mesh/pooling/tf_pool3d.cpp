...
REGISTER_OP("MeshWeightedPool3d")
    .Input("input: float32")      // batch_npoints * in_channels
    .Input("weight: float32")     // batch_npoints
    .Input("vt_replace: int32")   // batch_npoints
    .Input("vt_map: int32")       // batch_npoints
    .Input("vt_out: float32")     // batch_mpoints * D
    .Output("output: float32")    // batch_mpoints * in_channels
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle input, vt_out;
        TF_RETURN_IF_ERROR(c->WithRank(c->input(0), 2, &input));
        TF_RETURN_IF_ERROR(c->WithRank(c->input(4), 2, &vt_out));
        ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(vt_out,0),
                                                                          c->Dim(input,1)});
        c->set_output(0, output);
        return Status::OK();
    });
REGISTER_OP("MeshWeightedPool3dGrad")
    .Input("input: float32")       // batch_npoints * in_channels
    .Input("grad_output: float32") // batch_mpoints * in_channels
    .Input("weight: float32")      // batch_npoints
    .Input("vt_replace: int32")    // batch_npoints
...
class WeightedPool3dGpuOp : public OpKernel {
    public:
        explicit WeightedPool3dGpuOp(OpKernelConstruction* context) : OpKernel(context){}
        ...
        void Compute(OpKernelContext* context) override {
            ...
            const Tensor& vtReplace_tensor = context->input(2);
            const Tensor& vtMap_tensor     = context->input(3);
            const Tensor& vtOut_tensor     = context->input(4);
            ...
            float* output = &(output_flat(0));
            cudaMemset(output, 0, sizeof(float)*nvOut*C);
            weightedPool3dLauncher(nvIn, C, vtReplace, vtMap, input, weight, output);
        }
};
REGISTER_KERNEL_BUILDER(Name("MeshWeightedPool3d").Device(DEVICE_GPU), WeightedPool3dGpuOp);
...
class WeightedPool3dGradGpuOp : public OpKernel {
    public:
        explicit WeightedPool3dGradGpuOp(OpKernelConstruction* context) : OpKernel(context){}
        ...
        void Compute(OpKernelContext* context) override
        {
            ...
            const Tensor& vtReplace_tensor   = context->input(3);
            const Tensor& vtMap_tensor       = context->input(4);
            ...
            float* gradIn = &(grad_input_flat(0));
            cudaMemset(gradIn, 0, sizeof(float)*nvIn*C);
            weightedPool3dGradLauncher(nvIn, C, vtReplace, vtMap, gradOut, weight, gradIn);
        }
};
REGISTER_KERNEL_BUILDER(Name("MeshWeightedPool3dGrad").Device(DEVICE_GPU), WeightedPool3dGradGpuOp);