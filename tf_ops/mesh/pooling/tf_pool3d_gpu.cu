...
// input:     nvIn*C;
// output:    nvOut*C;
__global__ void max_pool3d_initialize(const int nvIn, const int C, const int* vtReplace, const int* vtMap,
...
// input:     nvIn*C;
// output:    nvOut*C;
__global__ void weighted_pool3d_forward(const int nvIn, const int C, const int* vtReplace, const int* vtMap,
                                        const float* input, const float* weight, float* output)
{
    ...
    if(vi < nvIn)
    {
        ...
        {
            int vo = vtMap[mapid]; // mapping from input index to the output index
            float w = weight[vi]; // weighted accumulation
            atomicAdd(&output[C*vo+ch],input[C*vi+ch]*w); // output should be initialized to zeros
        }
    }
}
...
// gradOutput: nvOut*C;
// gradInput:  nvIn*C;
__global__ void weighted_pool3d_backward(const int nvIn, const int C, const int* vtReplace, const int* vtMap,
                                         const float* gradOutput, const float* weight, float* gradInput)
{
    ...
    if(vi < nvIn)
    {
        ...
        if (vtMap[mapid]>=0)
        {
            int vo = vtMap[mapid]; // mapping from input index to the output index
            float w = weight[vi]; // weighted accumulation
            gradInput[C*vi+ch] = gradOutput[C*vo+ch]*w;
        }
    }
}
                       ...
                       const int* vtMap, const float* input, float* output, int* maxIndex)
{
    ...
    max_pool3d_initialize<<<numGrid,1024>>>(nvIn, C, vtReplace, vtMap, input, output);
    //cudaDeviceSynchronize();

    max_pool3d_forward<<<numGrid,1024>>>(nvIn, C, vtReplace, vtMap, input, output, maxIndex);
    //cudaDeviceSynchronize();
}
...
{
    int numGrid = int(nvIn*C/1024) + 1;
    weighted_pool3d_forward<<<numGrid,1024>>>(nvIn, C, vtReplace, vtMap, input, weight, output);
    //cudaDeviceSynchronize();
}
...
{
    int numGrid = int(nvIn*C/1024) + 1;
    weighted_pool3d_backward<<<numGrid,1024>>>(nvIn, C, vtReplace, vtMap, gradOutput, weight, gradInput);
    //cudaDeviceSynchronize();
}