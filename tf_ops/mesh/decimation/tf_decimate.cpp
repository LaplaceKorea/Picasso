...
REGISTER_OP("MeshDecimation")
  .Attr("shuffle_bins: int")         // number of bins to shuffle cost indices
  .Attr("use_area: bool")            // indicates if use area to weigh Quadric errors
  .Attr("wgt_bnd: float")            // penalty weight for boundary edges
  .Attr("wgt_consist: float")        // penalty weight for edges failed consistency check
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    c->WithRank(c->input(0), 2, &vertex_input);
    ::tensorflow::shape_inference::ShapeHandle face_input;
    c->WithRank(c->input(1), 2, &face_input);
    ::tensorflow::shape_inference::ShapeHandle normal_input;
    c->WithRank(c->input(2), 2, &normal_input);
...
REGISTER_OP("CountVertexAdjface")
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    c->WithRank(c->input(2), 2, &vertex_out);
...
REGISTER_OP("ComputeVertexGeometry")
  .Input("vertex: float32")           // concat_Nv*Dim(Dim>=3 at least xyz)
  .Input("face: int32")               // face vertex list: concat_NfIn*3
  .Input("face_geometry: float32")    // concat_Nf*4, [normal,area] of faces
  .Output("vertex_geometry: float32") // concat_Nv*4, [normal,area] of vertices
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ::tensorflow::shape_inference::ShapeHandle vertex_in;
    c->WithRank(c->input(0), 2, &vertex_in);

    // output shape setting
    ::tensorflow::shape_inference::ShapeHandle vertex_geometry = c->MakeShape({c->Dim(vertex_in, 0), 4});
    c->set_output(0, vertex_geometry);
    return Status::OK();
  });
...
class MeshDecimationGpuOp: public OpKernel{
    public:
        ...
        {
            OP_REQUIRES_OK(context, context->GetAttr("shuffle_bins", &shuffle_bins_));
            OP_REQUIRES(context, shuffle_bins_ > 0, errors::InvalidArgument("Mesh decimation requires shuffle_bins>0, got ", shuffle_bins_));
            ...
            OP_REQUIRES_OK(context, context->GetAttr("wgt_consist", &wgt_consist_));
            OP_REQUIRES(context, wgt_consist_ >= 0, errors::InvalidArgument("Mesh decimation requires wgt_consist>=0, got ", wgt_consist_));
        }
        void Compute(OpKernelContext * context) override {
            ...
            OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape{Nv,3}, &vertexOut_tensor));
            ...
            cudaMemset(vertexOut,0,sizeof(float)*Nv*3);
            ...
            meshDecimationLauncher(shuffle_bins_*B, use_area_, wgt_bnd_, wgt_consist_,
                                   B, Nv, Nf, nvIn, mfIn, nv2Remove, vertexIn, faceIn, planeIn,
...
class CombineClustersGpuOp: public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context) override {
            ...
            //std::cout<<"nvA, nvB: "<<nvA<<", "<<nvB<<std::endl;
...
class ComputeVertexGeometryGpuOp: public OpKernel{
    public:
        explicit ComputeVertexGeometryGpuOp(OpKernelConstruction* context):OpKernel(context) {}
        ...
        void Compute(OpKernelContext * context) override {
            ...
            const Tensor& vertexIn_tensor = context->input(0);
            const Tensor& faceIn_tensor = context->input(1);
            const Tensor& faceGeo_tensor = context->input(2);
            ...
            OP_REQUIRES(context, faceIn_tensor.dims()==2 && faceIn_tensor.shape().dim_size(1)==3,
                        errors::InvalidArgument("The shape of input face should be (NfIn, 3))."));
            OP_REQUIRES(context, faceGeo_tensor.dims()==2 && faceGeo_tensor.shape().dim_size(1)==4
                        && faceGeo_tensor.shape().dim_size(0)==faceIn_tensor.shape().dim_size(0),
                        errors::InvalidArgument("The shape of face geometry should be (NfIn, 4)."));

            // flatten the input tensors
            auto faceIn_flat = faceIn_tensor.flat<int32>();
            auto faceGeo_flat = faceGeo_tensor.flat<float>();

            // Create an output tensor
            Tensor* vertexGeo_tensor = NULL;
            OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape{Nv,4}, &vertexGeo_tensor));
            auto vertexGeo_flat = vertexGeo_tensor->flat<float>();

            const int* faceIn = &(faceIn_flat(0));
            const float* faceGeo = &(faceGeo_flat(0));

            float* vertexGeo = &(vertexGeo_flat(0));
            cudaMemset(vertexGeo,0,sizeof(float)*Nv*4); // initialize vertexGeo all to zeros
            computeVertexGeometryLauncher(Nf, faceIn, faceGeo, vertexGeo);
        }
};
REGISTER_KERNEL_BUILDER(Name("ComputeVertexGeometry").Device(DEVICE_GPU),ComputeVertexGeometryGpuOp);
